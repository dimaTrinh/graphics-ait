...
class Scene {
  constructor(gl) {
    ...
    this.objectPosition1 = new Vec3(0.7,0.0,0.0);
    this.objectPosition2 = new Vec3(-0.7,-0.7,0.0); 
    this.objectPosition3 = new Vec3(0.0, 0.7, 0.0);

    this.timeAtFirstFrame = new Date().getTime();
    this.timeAtLastFrame = this.timeAtFirstFrame;

    this.avatarVelocity = new Vec3(0.0,0.0,0.0);
    this.objectVelocity = new Vec3(0.0,-0.5,0.0);
  ...
  update(gl, keysPressed) {
   ...
   //Goes into VS, helps move the object around according to key pressed
    ...
    if(objectPositionHandle === null) {
      ...
      this.avatarPosition.commit(gl, objectPositionHandle);
    }
    ...
    this.objectPosition1.addScaled(timePassed, this.objectVelocity);

    //Helps make the object wrap around the screen
    if(Math.abs(this.objectPosition1.y) > 1){
      if (this.objectPosition1.y > 1){
        this.objectPosition1.y = -1.0;
      }
      else{
        this.objectPosition1.y = 1.0;
      }
    ...
    this.objectPosition1.commit(gl, objectPositionHandle);
    this.quadGeometry.draw();


    this.objectPosition2.addScaled(timePassed, this.objectVelocity);

    //Helps make the object wrap around the screen
    if(Math.abs(this.objectPosition2.y) > 1){
      if (this.objectPosition2.y > 1){
        this.objectPosition2.y = -1.0;
      }
      else{
        this.objectPosition2.y = 1.0;
      }
      this.objectVelocity.add(new Vec3(0.0, -0.1, 0.0));
    }

    this.objectPosition2.commit(gl, objectPositionHandle);  
    this.quadGeometry.draw();

    this.objectPosition3.addScaled(timePassed, this.objectVelocity);

    //Helps make the object wrap around the screen
    if(Math.abs(this.objectPosition3.y) > 1){
      if (this.objectPosition3.y > 1){
        this.objectPosition3.y = -1.0;
      }
      else{
        this.objectPosition3.y = 1.0;
      }
    ...
    this.objectPosition3.commit(gl, objectPositionHandle);  
    this.quadGeometry.draw();
